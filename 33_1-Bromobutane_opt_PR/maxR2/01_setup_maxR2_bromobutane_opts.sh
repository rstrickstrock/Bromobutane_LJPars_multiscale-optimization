...
#thisCWD="$(pwd)"
#echo $thisCWD
thisCWD="\/home\/rstric2s\/current_sim\/bromobutane.app\/33_1-Bromobutane_opt_PR\/01_C4Br_C3Br\/maxR2"
...
for optrun in bestTrainedModels/1-bromo*; do
  ...
  n="$(cut -d "-" -f2 <<<$optrun)"
  sed "s/#SBATCH --job-name=1-mR2-QQ/#SBATCH --job-name=1-mR2-$n/" <template.batch_run_maxR2.sh >batch.tmp
