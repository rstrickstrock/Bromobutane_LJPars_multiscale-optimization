...
class Simulations:
    ...
    def lj_parameters_gmx2amber(self, parameters, parameter_type):
        ...
        Log = self.__log
        Log.log("!!! WARNING: DID NOT APPLY PARAMETER CONVERSION TO AMBER.")
        Log.log("If parameter conversion should be applied check: Simulations.lj_parameters_gmx2amber()")
        Log.log("an REMOVE first hardcoded return block.")
        return parameters
    ...
    def get_simulation_results(self, result_dirs, eval_script, collect_script,  cleanup_script=None, eval_script_type='sh',
        ...
        if type(results) is list:
            if len(results) == 1 and type(results[0]) is list:
                try:
                    results = results[0]
                except:
                    if self.__debug:
                        Log.log("\t\t\tSimulations.get_simulation_results(): could not do results = results[0]")
        if self.__debug:
            Log.log("\t\tSimulations.get_simulation_results(): results = {}".format(results))
