...
for i in range(0, numberOfModels):
  ...
  #print(f'{thisRndInt}')
  thisDegree = str(int(minMAPERow.degree))
  #print(f'{thisDegree}')

  trainedModelFile = f'trained_model_{thisRatio}_{thisRndInt}_deg{thisDegree}.sav'
  pathToModelFile = os.path.join("trainedModels", thisRatio, trainedModelFile)
  pathToModelDest = os.path.join(modelDir, f'{substance}_minMAPE-{i}.sav')
  if not os.path.isfile(pathToModelFile):
    ...
    with open(logFile, "a") as f:
      f.write(f'{trainedModelFile} -> {substance}_minMAPE-{i}.pth\n')
