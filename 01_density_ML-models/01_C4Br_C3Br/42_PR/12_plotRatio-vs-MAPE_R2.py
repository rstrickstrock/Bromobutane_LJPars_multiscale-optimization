...
#print(f'{thisDF}')
#print(f'{thisDF["degree"].unique()}')
#print(f'{len(thisDF["degree"].unique())}')
#exit()

## plots
Degrees = thisDF["degree"].unique()
#print(f'{Degrees}')
#print(f'{len(Degrees)}')
...
for deg in Degrees:
  #print(f'deg: {deg}')  
  dfThisSubset = thisDF[(thisDF["degree"] == deg)]
...
for n in range(0,len(Degrees)):
  ...
  if deg >= 7:
    break
  ax1.errorbar(np.array(Ratios), np.array(MAPE[n]), yerr=np.array(MAPEerr[n]).T, label=f'polyn. degree: {int(deg)}', color=colors[n], ls='dotted', capsize=4.0, marker=markers[n])
...
if saveOrShow == "show":
    plt.show()
if saveOrShow == "save":
    plt.savefig(f'Ratio-vs-MAPE_polynDegs_{datasetname}.png', dpi=figDPI)
...
for n in range(0,len(Degrees)):
  ...
  if deg >= 7:
    break
  ax1.errorbar(np.array(Ratios), np.array(R2[n]), yerr=np.array(R2err[n]).T, label=f'polyn. degree: {int(deg)}', color=colors[n], ls='dotted', capsize=4.0, marker=markers[n])
...
if saveOrShow == "show":
    plt.show()
if saveOrShow == "save":
    plt.savefig(f'Ratio-vs-R2_polynDegs_{datasetname}.png', dpi=figDPI)
