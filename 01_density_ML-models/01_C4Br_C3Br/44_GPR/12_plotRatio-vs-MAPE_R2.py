...
#print(f'{thisDF}')
#print(f'{thisDF["kernel"].unique()}')
#print(f'{len(thisDF["kernel"].unique())}')
#exit()

## plots
Kernels = thisDF["kernel"].unique()
#print(f'{Kernels}')
#print(f'{len(Kernels)}')
if not allKernels:
  Kernels = ["Matern", "RQ"]
...
for kernel in Kernels:
  ...
  dfThisSubset = thisDF[(thisDF["kernel"] == kernel)]
...
for n in range(0,len(Kernels)):
  #print(f'{n}')
  kernel = np.array(Kernels)[n]
  if kernel == "Matern":
    kernelLabel = "Matérn"
  else:
    kernelLabel = kernel
  ax1.errorbar(np.array(Ratios), np.array(MAPE[n]), yerr=np.array(MAPEerr[n]).T, label=f'Kernel: {kernelLabel}', color=colors[n], ls='dotted', capsize=4.0, marker=markers[n])
...
if saveOrShow == "save":
    if not allKernels:
      plt.savefig(f'Ratio-vs-MAPE_Kernels_{datasetname}.png', dpi=figDPI)
    else:
      plt.savefig(f'Ratio-vs-MAPE_AllKernels_{datasetname}.png', dpi=figDPI)
...
for n in range(0,len(Kernels)):
  ...
  if kernel == "Matern":
    ...
    kernelLabel = kernel
  ax1.errorbar(np.array(Ratios), np.array(R2[n]), yerr=np.array(R2err[n]).T, label=f'Kernel: {kernelLabel}', color=colors[n], ls='dotted', capsize=4.0, marker=markers[n])
...
if saveOrShow == "save":
    if not allKernels:
      plt.savefig(f'Ratio-vs-R2_Kernels_{datasetname}.png', dpi=figDPI)
    else:
      plt.savefig(f'Ratio-vs-R2_AllKernels_{datasetname}.png', dpi=figDPI)
