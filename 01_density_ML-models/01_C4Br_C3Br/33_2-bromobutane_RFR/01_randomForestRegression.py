...
data = pd.read_csv('2-bromobutane_trainingsdata.csv')
...
for thisRatio in testSizes:
  ...
  for rndInt in rndInts:
    ...
    for thisNumTrees in numberOfTrees:
      #print(f'.')
      ## create the model
      model = RandomForestRegressor(n_estimators=thisNumTrees, random_state=rndInt)
      
      ## train/fit the model
      model.fit(X_train, Y_train)
      pickle.dump(model, open(f'trained_model_{thisRatio}_{rndInt}_nTrees-{thisNumTrees}.sav', 'wb'))
