...
#thisCWD="$(pwd)"
#echo $thisCWD
thisCWD="\/home\/rstric2s\/current_sim\/bromobutane.app\/33_2-Bromobutane_opt_PR\/01_C4Br_C3Br\/minMAPE"
...
for optrun in bestTrainedModels/2-bromo*; do
  ...
  n="$(cut -d "-" -f2 <<<$optrun)"
  sed "s/#SBATCH --job-name=2-mM-QQ/#SBATCH --job-name=2-mM-$n/" <template.batch_run_minMAPE.sh >batch.tmp
